...
function showSingleCluster(cid, highlightGene) {{
    ...
    // One per-gene top-K cap serves both purposes: it's sparse enough that label
    // propagation still finds real sub-groups instead of collapsing into one giant
    // community, while (unlike a global top-K-by-weight cut) every gene keeps its
    // own strongest few connections, so the rendered graph matches the cluster's
    // true connectivity instead of an artifact of the cutoff. K=10 was verified to
    // already eliminate stray isolated nodes across the largest Leiden clusters;
    // going higher roughly doubles edge count (and layout time) for little gain.
    const K_NEAREST = 10;
    const nearestEdges = Array.from(topKEdgeMap(K_NEAREST).values());
    const edgeList = nearestEdges;
