        ...
        <div style="padding:10px 20px; border-bottom:1px solid #1e293b; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; background:#0b1120; font-size:12px; flex-shrink:0;">
            <div style="display:flex; align-items:center; gap:14px; flex-wrap:wrap;">
                ...
                <div style="display:flex; align-items:center; gap:5px;">
                    ...
                    <button id="export-theme-light" class="btn" style="padding:3px 10px; font-size:11px;" onclick="setExportSlideTheme('light')">☀️ Light (Slide Friendly)</button>
...
function renderTreeSidebar() {{
    ...
    geneInfoEl.innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:4px;">
            <h2 style="margin:0; font-size:15px; color:${{isDark ? '#f8fafc' : '#0f172a'}};">🌳 Co-Loss Matrix</h2>
            ${{countBadge}}
        </div>
            ...
            <div style="padding:14px; font-size:12px;">
                <div style="padding:10px 12px; background:${{isDark ? '#161d31' : '#f1f5f9'}}; border:1px solid ${{col}}; border-radius:8px;">
                    <div style="display:flex; align-items:center; justify-content:space-between;">
                        ...
                        <span style="font-size:11px; color:#8892b0; cursor:pointer;" onclick="removeTreeGene('${{g}}')" title="Remove gene">✕</span>
                    </div>
                ...
                <div style="margin-top:14px; padding:10px; background:rgba(56,189,248,0.1); border:1px solid rgba(56,189,248,0.25); border-radius:6px; font-size:11.5px; color:${{isDark ? '#93c5fd' : '#0369a1'}}; line-height:1.4;">
                    💡 <strong>Add at least 1 more gene</strong> to display the pairwise co-loss matrix. Use the search bar above or choose from recommended partners below.
                </div>
    ...
    TREE_SELECTED_GENES.forEach((gRow, i) => {{
        ...
        TREE_SELECTED_GENES.forEach((gCol, j) => {{
            if (i === j) {{
                ...
                const bg = jaccard <= 0.001 
                    ? (isDark ? '#0c1220' : '#ffffff') 
                    : getJaccardColor(jaccard);
                const textCol = jaccard <= 0.001 
                    ? '#64748b' 
                    : '#ffffff';
                ...
                rowCells += `
                    <td style="padding:6px 4px; text-align:center; font-family:ui-monospace, monospace; font-size:11.5px; font-weight:700; color:${{textCol}}; background:${{bg}}; opacity:${{opacity}}; border:1px solid ${{isDark ? '#1e293b' : '#e2e8f0'}}; cursor:pointer; transition:transform 0.1s;" 
                        title="${{gRow}} ↔ ${{gCol}}: Jaccard = ${{jaccard.toFixed(3)}} (Click to view interaction)"
    ...
    partnerListEl.innerHTML = `
        <div style="padding:12px 10px;">
            <div style="font-size:10.5px; font-weight:700; color:#8892b0; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px; display:flex; justify-content:space-between; align-items:center;">
                <span>Pairwise Co-Loss Matrix:</span>
                <span style="font-weight:400; font-size:10px; color:#38bdf8;">Click cell for interface</span>
            </div>
...
async function generatePresentationSlideCanvas(options = {{}}) {{
    ...
    if (isTree) {{
        // ---- Species Tree Pairwise Co-Loss Matrix Slide Panel ----
        ...
        ctx.fillStyle = P.textSecondary;
        ctx.fillText('Jaccard similarity on independent Dollo loss branches (196 genomes)', sx + 35, sy + 70);
        ...
        if (nGenes >= 2) {{
            ...
            TREE_SELECTED_GENES.forEach((gRow, i) => {{
                ...
                TREE_SELECTED_GENES.forEach((gCol, j) => {{
                    ...
                    if (i === j) {{
                        ...
                        ctx.textAlign = 'center';
                        ctx.fillText(jaccard.toFixed(2), cX + cW / 2, cY + cH / 2 + 5);
                    }}
                }});
            }});
            ctx.textAlign = 'left';
            ...
            ctx.font = '600 12px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
            ctx.fillStyle = P.textSecondary;
            ctx.fillText('CO-LOSS SIMILARITY (JACCARD)', legX + 20, legY + 28);

            ctx.font = '600 11px ui-monospace, monospace';
            ctx.fillStyle = P.textMuted;
            ctx.fillText('0.20', barX, barY + 22);
            ctx.fillText('≥0.65', barX + barW - 28, barY + 22);
