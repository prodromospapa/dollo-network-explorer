...
let TREE_MATRIX_METRIC = 'distance'; // default: 'distance' (Tree distance) or 'coloss' (Jaccard similarity)
...
let TREE_BRANCH_LEN_MODE = 'phylogram'; // default: use evolutionary branch lengths
...
async function generatePresentationSlideCanvas(options = {{}}) {{
    ...
    if (isTree) {{
        ...
        if (nGenes >= 2) {{
            ...
            // 3. Compact Tree Distance Scale Indicator below Grid (no bulky box)
            const scaleY = gridStartY + nGenes * cellSize + 22;
            const barW = Math.min(320, actualGridW);
            const barX = gridStartX + (actualGridW - barW) / 2;
            
            ctx.font = '11px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
            ctx.fillStyle = P.textMuted;
            ctx.textAlign = 'left';
            ctx.fillText('0.00 (Coupled)', barX, scaleY - 6);
            ctx.textAlign = 'right';
            ctx.fillText('1.00 (Divergent)', barX + barW, scaleY - 6);
            ctx.textAlign = 'center';
            ctx.fillStyle = P.textSecondary;
            ctx.fillText('Tree Distance Scale', barX + barW / 2, scaleY - 6);
            ...
            drawCanvasRoundedRect(ctx, barX, scaleY, barW, 6, 3);
            ctx.fillStyle = grad;
            ctx.fill();
            ctx.textAlign = 'left';

            // 4. Gene Status Summary Table
            const sumStartY = scaleY + 36;
