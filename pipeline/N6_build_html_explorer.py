...
<div id="header">
    ...
    <div class="controls" id="header-controls">
        ...
        <div id="controls-cluster" style="display:none; align-items:center; gap:10px;">
            <label>Filter:</label>
            <select id="gene-filter-cluster" onchange="applyGeneFilterChange(this.value);" style="background:#0d1220; border:1px solid #3a4570; color:#e0e6f0; padding:3px 6px; border-radius:4px; font-size:11px;">
                <option value="all">All Genes</option>
...
// Shared by both the Network View filter (#gene-filter) and the Leiden Modules
// filter (#gene-filter-cluster) — they show/hide together depending on mode, but
// both need to update the same geneFilterMode and re-render whichever view is live.
function applyGeneFilterChange(mode) {{
    geneFilterMode = mode;
    const mainSelect = document.getElementById('gene-filter');
    const clusterSelect = document.getElementById('gene-filter-cluster');
    if (mainSelect && mainSelect.value !== mode) mainSelect.value = mode;
    if (clusterSelect && clusterSelect.value !== mode) clusterSelect.value = mode;
...
function setSidebarTab(tab, opts) {{
    // switchView() passes skipRender: true when it's about to trigger the equivalent
    // content render itself right afterward (e.g. showAllClusters() /
    // renderSingleClusterContent()) — without this, both would run per mode switch.
    const skipRender = Boolean(opts && opts.skipRender);
    ...
    if (tab === 'matrix') {{
        ...
        if (!skipRender) renderTreeSidebar();
    }} else {{
        ...
        if (geneToLoad) {{
            ...
            renderSingleClusterContent(currentClusterId);
        }} else {{
            renderClusterDirectory();
        }}
    }}
...
function switchView(mode) {{
    ...
    if (mode === 'gene' || mode === 'network') {{
        ...
        setSidebarTab('list', {{ skipRender: true }});
        if (mode === 'all_clusters') {{
            showAllClusters();
        }} else if (mode === 'single_cluster' && currentClusterId !== null) {{
            renderSingleClusterContent(currentClusterId, PENDING_CLUSTER_HIGHLIGHT);
            PENDING_CLUSTER_HIGHLIGHT = null;
        }} else {{
            showAllClusters();
        }}
        updateStatus();
...
// Set by showSingleCluster() right before it drives a mode switch, so the highlight
// gene survives the switchView -> setSidebarTab -> renderSingleClusterContent chain.
let PENDING_CLUSTER_HIGHLIGHT = null;

function showSingleCluster(cid, highlightGene) {{
    if (!CLUSTERS[cid]) return;
    if (currentMode !== 'single_cluster') {{
        // Not in cluster mode yet: switchView() below will call
        // renderSingleClusterContent() itself once the mode/UI is set up. Doing the
        // heavy render here too (or letting setSidebarTab do it a third time, as this
        // used to) would redo the same expensive work 2-3x per click.
        currentClusterId = cid;
        clusterFocusedGene = null;
        PENDING_CLUSTER_HIGHLIGHT = highlightGene || null;
        switchView('single_cluster');
        return;
    }}
    renderSingleClusterContent(cid, highlightGene);
}}

function renderSingleClusterContent(cid, highlightGene) {{
    if (!CLUSTERS[cid]) return;
    currentClusterId = cid;
    clusterFocusedGene = null;
    ...
    // Sort each gene's own intra-cluster partners once, strongest first. Everything
    // below is built from per-gene top-K slices of this rather than a single global
    // top-K-by-weight cut: a global cut let a few very strongly auto-correlated genes
    // hog most of the edge budget and left everyone else with zero rendered edges,
    // which fractured genuinely single-component clusters into 100+ disconnected
    // islands in the force layout (e.g. one cluster went from 1 component to 181,
    // 118 of them totally isolated nodes) even though the underlying data was one
    // connected community.
    const sortedPartnersOf = new Map();
    shown.forEach(name => {{
        const d = getGeneData(name);
        const sorted = d ? d.p.filter(p => p.j > 0 && p.n !== name && shownSet.has(p.n)).sort((x, y) => y.j - x.j) : [];
        sortedPartnersOf.set(name, sorted);
    }});
    function topKEdgeMap(k) {{
        const map = new Map();
        shown.forEach(name => {{
            sortedPartnersOf.get(name).slice(0, k).forEach(p => {{
                const a = name < p.n ? name : p.n;
                const b = name < p.n ? p.n : name;
                const key = a + '\\u0000' + b;
                const existing = map.get(key);
                if (!existing || p.j > existing.j) map.set(key, {{ a, b, j: p.j }});
            }});
        }});
        return map;
    }}

    // Render every stored intra-cluster pair — no Jaccard threshold, no top-N cap.
    // Leiden clustering itself was computed on the complete similarity graph (every
    // pair >= 0.05, no per-gene limit); network_partners.bin (what getGeneData().p
    // draws from) already only keeps each gene's strongest 500 partners at
    // Jaccard >= 0.08 for the main ego-network browser, which is a narrower view
    // than what the clustering saw. Capping further on top of that (as done
    // previously) is what fragmented genuinely single-component clusters into many
    // disconnected islands - so this shows everything the site's data has for the
    // cluster, at the cost of a much longer render/layout time for the 1-2 largest
    // modules (~2000 genes, 300k+ edges can take under a minute).
    const edgeList = Array.from(topKEdgeMap(Infinity).values());
    ...
    // so the layout the physics already produces is actually legible. This is purely
    // an internal signal for node color, not a filter — every edge above is still
    // rendered regardless of what label propagation decides here. Label propagation
    // over the full graph collapses into one giant community on these densely-
    // connected modules (nearly everything is at least weakly linked to everything),
    // so detect structure on a sparser per-gene top-10 graph instead.
    const lpaEdges = Array.from(topKEdgeMap(10).values());
...
function renderEdgeAlignmentModalContent() {{
    ...
    if (profile) {{
        ...
        profile.species.forEach(sp => {{
            ...
            rowsHtml += `
                <div style="display:flex; align-items:center; margin-bottom:8px; font-family:ui-monospace, SFMono-Regular, monospace; font-size:12px; border-bottom:1px solid #1e293b; padding-bottom:6px;">
                    ...
                    <div class="ea-seq-track" style="flex:1; min-width:0; overflow-x:auto; letter-spacing:1.5px; line-height:1.5; white-space:nowrap; background:#0b1120; padding:4px 8px; border-radius:4px; border:1px solid #1e293b;">
                        ${{coloredSeq}}
                    </div>
        ...
        // Each species' sequence track scrolls independently by default, which breaks
        // the alignment (same horizontal offset = same alignment column) across rows
        // whenever the region is wide enough to need scrolling, especially Full Track.
        // Scrolling any one row drives every other row to the same position.
        const seqTracks = bodyEl.querySelectorAll('.ea-seq-track');
        let syncingScroll = false;
        seqTracks.forEach(track => {{
            track.addEventListener('scroll', () => {{
                if (syncingScroll) return;
                syncingScroll = true;
                seqTracks.forEach(other => {{
                    if (other !== track) other.scrollLeft = track.scrollLeft;
                }});
                syncingScroll = false;
            }});
        }});
