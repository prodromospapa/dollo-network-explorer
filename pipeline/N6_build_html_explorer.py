...
async function generatePresentationSlideCanvas(options = {{}}) {{
    ...
    if (isTree) {{
        ...
        ctx.fillStyle = P.textPrimary;
        ctx.fillText('Species Tree & Gene Status', sx + 25, sy + 38);

        ctx.font = '13.5px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
        ctx.fillStyle = P.textSecondary;
        ctx.fillText(`Tracking ${{TREE_SELECTED_GENES.length}} gene${{TREE_SELECTED_GENES.length === 1 ? '' : 's'}} across 196 eukaryotic genomes`, sx + 25, sy + 62);

        let chipY = sy + 88;
        if (TREE_SELECTED_GENES.length === 0) {{
            ctx.font = 'italic 13px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
            ctx.fillStyle = P.textMuted;
            ctx.fillText('No genes currently on tree (add from sidebar or search)', sx + 25, chipY + 10);
        }} else {{
            TREE_SELECTED_GENES.slice(0, 4).forEach((g, idx) => {{
                const col = TREE_PALETTE[idx % TREE_PALETTE.length];
                const losses = GM[g] !== undefined ? GM[g] : '—';
                ...
                ctx.beginPath();
                ctx.arc(sx + 34, chipY + 4, 6, 0, 2 * Math.PI);
                ctx.fillStyle = col;
                ctx.fill();

                ctx.font = 'bold 15px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
                ctx.fillStyle = P.textPrimary;
                ctx.fillText(g, sx + 48, chipY + 9);
                const gW = ctx.measureText(g).width;

                ctx.font = '13px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif';
                ctx.fillStyle = P.textSecondary;
                ctx.fillText(`•  ${{losses}} Dollo losses  •  Present in ${{presCount}}/196 species (${{presPct}}%)`, sx + 48 + gW + 10, chipY + 9);
