...
#sidebar-search:focus {{ border-color: #38bdf8; }}
...
#sidebar-search-clear:hover {{ color: #e2e8f0; }}
#gene-info {{
    padding: 12px;
    border-bottom: 1px solid #2a3050;
    min-height: 70px;
...
<div id="main">
    ...
    <div id="sidebar">
        ...
        <div id="sidebar-search-wrap">
            ...
            <input id="sidebar-search" type="search" placeholder="Filter list…" autocomplete="off" spellcheck="false">
            <button id="sidebar-search-clear" title="Clear filter">×</button>
        </div>
...
function withGene(name, callback) {{
    ...
    fetchGeneData(name)
        .then(data => {{
            // Also fetch the partners we're about to render so we get cross-links
            const thresh = document.getElementById('thresh') ? parseFloat(document.getElementById('thresh').value) : 0.2;
            let topn = 25;
            if (document.getElementById('toggle-topn-max') && document.getElementById('toggle-topn-max').checked) {{
                topn = Infinity;
            }} else if (document.getElementById('topn')) {{
                topn = parseInt(document.getElementById('topn').value);
            }}
            const activeSet = getActiveGeneSet();
            const qualifying = data.p.filter(p => p.j >= thresh && (!activeSet || activeSet.has(p.n)));
            const filtered = qualifying.slice(0, topn);
            
            const fetches = filtered.map(p => fetchGeneData(p.n).catch(e => null));
...
function renderNetwork(geneName) {{
    ...
    if (window.clearSidebarSearch) clearSidebarSearch();
...
function showAllClusters(isFilterUpdate) {{
    ...
    selectedGene = null;
    searchInput.value = '';
    if (!isFilterUpdate && window.clearSidebarSearch) clearSidebarSearch();
...
(function() {{
    ...
    function applySidebarFilter() {{
        ...
        // Partner rows (.partner[data-gene])
        const partnerRows = document.querySelectorAll('#partner-list .partner[data-gene]');
        if (partnerRows.length) {{
            partnerRows.forEach(function(row) {{
                const name = (row.dataset.gene || '').toLowerCase();
                row.style.display = (!q || name.includes(q)) ? '' : 'none';
            }});
            return;
        }}

        // Cluster rows (.cluster-entry[data-cluster-id])
        document.querySelectorAll('#partner-list .cluster-entry').forEach(function(row) {{
            const cid = row.dataset.clusterId || '';
            const cname = (row.querySelector('.cname') ? row.querySelector('.cname').textContent : '').toLowerCase();
            const match = !q || ('c' + cid).includes(q) || cname.includes(q);
    ...
    // Expose helper so renderNetwork / showAllClusters can reset the filter
    window.clearSidebarSearch = function() {{
        sSearch.value = '';
        ssClear.style.display = 'none';
    }};
}})();
