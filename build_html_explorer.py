...
<div id="header">
    ...
    <div class="view-toggle">
        <button id="btn-view-whole" class="view-btn active" onclick="switchView('whole')" title="View all 11,236 genes and 73,467 edges with 60 FPS WebGL">🌐 Whole Network (WebGL)</button>
        <button id="btn-view-cy" class="view-btn" onclick="switchView('gene')" title="Interactive single-gene ego network & pathways">🔬 Gene Focus (Cytoscape)</button>
    </div>

    <button id="btn-all-clusters-head" class="btn-clusters-header" onclick="showAllClusters()">🔬 All Clusters (Leiden)</button>

    <div class="search-box">
        <input type="text" id="search" placeholder="Search gene (e.g. SCAPER, CEP290)..." autocomplete="off" spellcheck="false">
        <div id="suggestions"></div>
    </div>

    <div class="controls" id="explorer-controls">
        <label>Jaccard ≥ <span id="thresh-val">0.20</span></label>
        <input type="range" id="thresh" min="0.05" max="0.80" step="0.01" value="0.20">

        <label>Show Top <span id="topn-val">25</span></label>
        <input type="range" id="topn" min="5" max="100" step="5" value="25">
        <label style="font-size:11px; display:inline-flex; align-items:center; gap:3px;">
            <input type="checkbox" id="toggle-topn-max"> Max
        </label>

        <label style="margin-left:4px;">Filter:</label>
        <select id="gene-filter" style="padding:4px 8px; border:1px solid #3a4570; border-radius:5px; background:#0d1220; color:#e0e6f0; font-size:12px;">
            <option value="all">All Genes</option>
            <option value="all_ciliary">Curated Ciliary (Union)</option>
            <option value="syscilia_v2">SYSCILIA Gold Standard v2</option>
            <option value="ciliacarta">CiliaCarta (935 genes)</option>
            <option value="shared_core">Shared Core (SCGSv2 ∩ CiliaCarta)</option>
        </select>
...
<div id="main">
    ...
    <div id="sidebar">
        <div id="sidebar-search-wrap">
            ...
            <input type="text" id="sidebar-search" placeholder="Filter sidebar genes..." autocomplete="off">
...
async function loadWholeGraph() {{
    try {{
        ...
        sigmaRenderer.on('enterNode', ({{ node }}) => {{
            ...
            tooltip.innerHTML = `<strong>${{d.label}}</strong>${{getCiliaBadgeHtml(d.label)}}<br>Losses: ${{d.losses}}<br>Cluster C${{d.clusterId}}: ${{CLUSTER_NAMES[d.clusterId] || ''}}`;
    ...
    for (let i = 0; i < count; i++) {{
        ...
        if (pName) {{
            partners.push({{
                n: pName,
                j: jRaw / 65535.0,
                l: PARTNER_LOSSES[pIdx]
            }});
        }}
    }}
    const res = {{ l: PARTNER_LOSSES[idx], p: partners }};
    G_CACHE[name] = res;
    return res;
}}
...
function renderSidebar(name) {{
    ...
    document.getElementById('gene-info').innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; gap:8px;">
            <h2 style="display:flex; align-items:center; gap:6px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                ${{name}} ${{getCiliaBadgeHtml(name)}}
            </h2>
            <a href="${{getUniProtUrl(name)}}" target="_blank" rel="noopener noreferrer" class="gene-ext-link">UniProt ↗</a>
        </div>
        <div class="meta">
    ...
    qualifying.forEach((p, i) => {{
        ...
        <div class="partner" onclick="selectGene('${{p.n}}')">
            ...
            <a href="${{getUniProtUrl(p.n)}}" target="_blank" rel="noopener noreferrer" class="partner-ext-link" onclick="event.stopPropagation();">↗</a>
        </div>`;
    }});

    document.getElementById('partner-list').innerHTML = sideHtml || '<div style="padding:16px; color:#556; text-align:center;">No partners above current threshold.</div>';
}}
...
function renderEgoNetwork(geneName) {{
    ...
    // Instant cross-links between displayed partners (< 0.1 ms)
    qualifying.forEach(p => {{
        const pData = getGeneData(p.n);
        if (!pData) return;
        for (const pp of pData.p) {{
            if (pp.j >= thresh && displayedSet.has(pp.n) && pp.n !== geneName && p.n < pp.n) {{
                elements.push({{
                    group: 'edges',
                    data: {{
                        id: `${{p.n}}--${{pp.n}}`,
...
function updateStatus() {{
    ...
    if (currentMode === 'whole') {{
        ...
        status.textContent = `Mode: Cytoscape • ${{cy.nodes().length}} nodes • ${{cy.edges().length}} edges`;
    }}
}}
