...
function initCy() {{
    ...
    cy.on('mouseover', 'edge', function(evt) {{
        ...
        tipEl.innerHTML = `<strong>${{d.source}} ⟷ ${{d.target}}</strong><br>Jaccard Similarity: <strong>${{d.jaccard.toFixed(2)}}</strong>`;
        tipEl.style.display = 'block';
    }});
    ...
    qualifying.forEach(p => {{
        ...
        elements.push({{
            ...
            data: {{
                ...
                jaccard: p.j,
                width: 1.2 + p.j * 4.5,
                opacity: Math.min(0.88, 0.25 + p.j * 0.9),
                color: '#38bdf8'
            }}
        }});
    }});
            ...
            if (pp.j >= thresh && displayedSet.has(pp.n) && p.n < pp.n && pp.n !== geneName) {{
                elements.push({{
                    ...
                    data: {{
                        ...
                        jaccard: pp.j,
                        width: 0.8 + pp.j * 3.0,
                        opacity: Math.min(0.65, 0.15 + pp.j * 0.7),
                        color: 'rgba(90, 142, 255, 0.45)'
                    }}
                }});
            }}
        }}
